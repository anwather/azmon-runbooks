# SQL Server — SQL error 832 constant page changed in cache

**Alert ID:** `sql-error-832-constant-page-failure`
**Severity:** 1 Critical
**Category:** Storage
**Detection mechanism:** Custom Text Logs (ERRORLOG) DCR
**Source SCOM monitor / rule (if any):** `EventRule.DBEngine.Check_sum_failure_while_page_in_cache_id832`

---

## Symptom

The proposed alert fires when SQL ERRORLOG ingestion sees error 832. Typical message text: "A page that should not have changed in cache was modified unexpectedly."
...
- Microsoft docs: https://learn.microsoft.com/sql/relational-databases/errors-events/mssqlserver-832-database-engine-error
