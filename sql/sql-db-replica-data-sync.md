# SQL Server — Availability database data synchronization is unhealthy

**Alert ID:** `sql-db-replica-data-sync`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.DatabaseReplica.DataSynchronizationState`

---

## Symptom

An availability database is not synchronized or synchronizing as expected.

## What it means

A database replica is behind, suspended, or not joined. RPO increases and automatic failover can be blocked for synchronous databases.
