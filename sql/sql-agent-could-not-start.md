# SQL Server — SQL Server Agent could not be started

**Alert ID:** `sql-agent-could-not-start`
**Severity:** 1 Critical
**Category:** Agent
**Detection mechanism:** DCR-Events XPath
**Source SCOM monitor / rule (if any):** `CollectionRule.Agent.SQLServerAgent_could_not_be_started_1_5_Rule`
...
Agent failed during startup before it could schedule jobs or process alerts. The cause is usually permissions, service account, msdb access, or inability to connect to the engine.
...
| where Source has "SQLSERVERAGENT" and EventID == 103
