# SQL Server — SQL Agent job step subsystem failed to load

**Alert ID:** `sql-agent-job-step-subsystem-load-fail`
**Severity:** 1 Critical
**Category:** Agent
**Detection mechanism:** DCR-Events XPath
**Source SCOM monitor / rule (if any):** `CollectionRule.Agent.Job_step_cannot_be_run_because_the_subsystem_failed_to_load_1_5_Rule`

---

## Symptom

A job step cannot run because the requested subsystem failed to load.

## What it means

Agent could not initialize a subsystem such as CmdExec, PowerShell, SSIS, Replication, Analysis Services, or T-SQL proxy support.
...
| where Source has "SQLSERVERAGENT" and EventID == 212
