# SQL Server — Availability group replica role state is unhealthy

**Alert ID:** `sql-ag-replica-role-state`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityGroupHealth.AvailabilityReplicaRoleState`

---

## Symptom

At least one availability replica is not in an expected PRIMARY or SECONDARY role.

## What it means

A replica can be RESOLVING or UNKNOWN during failover, cluster transitions, or quorum loss. Client routing and data protection can be affected.
...
| extend InstanceName = tostring(P.payload.instanceName), DatabaseName = tostring(P.payload.databaseName)
    // Spans CheckId 140/141/142; narrow to one with: | where toint(P.checkId) == 140
