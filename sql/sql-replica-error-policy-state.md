# SQL Server — Availability replica user policy error

**Alert ID:** `sql-replica-error-policy-state`
**Severity:** 2 Error
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityReplicaErrorPolicy.State`

---

## Symptom

A custom PBM policy for an availability replica evaluated to error.

## What it means

The customer-defined replica policy has failed. Impact depends on the policy, commonly endpoint, backup preference, or replica configuration compliance.
