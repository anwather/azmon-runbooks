# SQL Server — Availability group replicas are not all connected

**Alert ID:** `sql-ag-replicas-connected`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityGroupHealth.AllAvailabilityReplicasAreConnected`

---

## Symptom

One or more replicas in the availability group are disconnected from the primary.

## What it means

AG data movement requires connected endpoints. Disconnected replicas stop receiving log blocks and can lose automatic failover readiness.
...
| extend InstanceName = tostring(P.payload.instanceName), DatabaseName = tostring(P.payload.databaseName)
    // Spans CheckId 140/141/142; narrow to one with: | where toint(P.checkId) == 141
