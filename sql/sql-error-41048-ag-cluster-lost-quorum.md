# SQL Server — SQL error 41048 WSFC lost quorum

**Alert ID:** `sql-error-41048-ag-cluster-lost-quorum`
**Severity:** 1 Critical
**Category:** AlwaysOn
**Detection mechanism:** Custom Text Logs (ERRORLOG) DCR
**Source SCOM monitor / rule (if any):** `EventRule.AvailabilityGroup.ClusterLostQuorum_1_5_Rule`

---

## Symptom

The proposed alert fires when SQL ERRORLOG ingestion sees error 41048. Typical message text: "The local availability replica cannot communicate with the WSFC resource due to lost quorum or connectivity."
...
- Microsoft docs: https://learn.microsoft.com/sql/relational-databases/errors-events/mssqlserver-41048-database-engine-error
