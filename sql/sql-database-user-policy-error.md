# SQL Server — Database user policy error

**Alert ID:** `sql-database-user-policy-error`
**Severity:** 2 Error
**Category:** Database
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.DatabaseErrorUserPolicy.DBErrorUserPolicyState`

---

## Symptom

A custom Policy-Based Management policy for a database evaluated to error.

## What it means

The customer-defined database policy has failed. Impact depends on the policy condition, but it usually indicates a database configuration, availability, security, or operational standard violation.
