# SQL Server — Availability group user policy error

**Alert ID:** `sql-ag-user-policy-error`
**Severity:** 2 Error
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityGroupErrorPolicy.State`

---

## Symptom

A custom Policy-Based Management policy for the availability group evaluated to error.

## What it means

The customer-defined AG policy has failed. Impact depends on the policy condition, but it usually indicates an AG configuration or operational rule violation.
