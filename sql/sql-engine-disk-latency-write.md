# SQL Server — DB Engine disk write latency is high

**Alert ID:** `sql-engine-disk-latency-write`
**Severity:** 2 Error
**Category:** Storage
...
Average SQL volume write latency is above the proposed threshold for multiple samples.

## What it means

Log and data writes are slow. Commit latency, tempdb allocations, checkpoint, backup, and AG send/redo can all degrade when WRITELOG or IO_COMPLETION waits rise.

## Common causes

- WRITELOG pressure from slow log volume
- Autogrowth zeroing data files without instant file initialization
- Synchronous AG commit waiting on secondary hardening
- Checkpoint or tempdb allocation burst
- Storage controller/cache issue
...
| where CounterName =~ "Avg. Disk sec/Write" or InstanceName has "sqlservr"
