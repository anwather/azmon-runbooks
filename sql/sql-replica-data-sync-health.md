# SQL Server — Availability replica data synchronization is unhealthy

**Alert ID:** `sql-replica-data-sync-health`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityReplica.DataSynchronizationHealth`

---

## Symptom

The local availability replica reports unhealthy database synchronization.

## What it means

One or more databases on this replica are not synchronizing as expected, often because data movement is suspended, the endpoint is disconnected, or redo/send queues are growing.
...
| where EventID in (7140, 7141, 7142, 7440, 7441, 7442, 7740, 7741, 7742)
...
| extend InstanceName = tostring(P.payload.instanceName), DatabaseName = tostring(P.payload.databaseName)
    // Spans CheckId 140/141/142; narrow to one with: | where toint(P.checkId) == 142
...
- KQL: `packs/sql/docs/kql/probe-availability-replica.kql`
- Bicep: `packs/sql/alerts/probes.bicep`
- Probe script: `scripts/packs/sql/Public/Test-SqlAvailabilityReplica.ps1`
