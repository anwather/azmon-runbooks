# SQL Server — Availability database data movement is suspended

**Alert ID:** `sql-db-replica-suspend`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.DatabaseReplica.SuspendState`

---

## Symptom

Data movement is suspended for an availability database.

## What it means

The AG database is not sending or applying log records. It remains stale until resumed and caught up.
...
| extend InstanceName = tostring(P.payload.instanceName), DatabaseName = tostring(P.payload.databaseName)
    // Spans CheckId 150/151/152; narrow to one with: | where toint(P.checkId) == 151
