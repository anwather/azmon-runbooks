# SQL Server — Availability group automatic failover is not ready

**Alert ID:** `sql-ag-automatic-failover-readiness`
**Severity:** 2 Error
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityGroupHealth.AutomaticFailoverReadiness`

---

## Symptom

No suitable synchronous automatic-failover partner is ready for the availability group.

## What it means

Automatic failover may not occur during a primary outage because the secondary is not synchronized, not healthy, or not configured for automatic failover.
...
| where EventID in (7130, 7430, 7730)
...
- KQL: `packs/sql/docs/kql/probe-availability-group.kql`
- Bicep: `packs/sql/alerts/probes.bicep`
- Probe script: `scripts/packs/sql/Public/Test-SqlAvailabilityGroup.ps1`
