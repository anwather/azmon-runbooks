# SQL Server — SQL Agent job duration is over threshold

**Alert ID:** `sql-agent-job-duration`
**Severity:** 2 Error
**Category:** Agent
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AgentJob.Duration`

---

## Symptom

A SQL Agent job is currently running longer than its configured warning or critical duration.

## What it means

The job may be blocked, waiting on I/O, processing unusually large data, hung in an external subsystem, or running with an unrealistic threshold.

## Common causes

- Blocking or wait on long-running transaction
- Input volume larger than normal
- Subsystem process hung or waiting on external resource
- Statistics/plan regression causing slow query
- Threshold not tuned for expected maintenance window
...
| where EventID in (7271, 7571, 7871)
...
- KQL: `packs/sql/docs/kql/probe-agent-job-duration.kql`
