# SQL Server — Synchronous AG replicas are not synchronized

**Alert ID:** `sql-ag-sync-replicas-data-sync`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.AvailabilityGroupHealth.AvailabilityReplicasSynchronizationState`

---

## Symptom

A synchronous-commit replica or database is not synchronized.

## What it means

Synchronous replicas must reach SYNCHRONIZED for zero-data-loss failover. If they are synchronizing or not synchronizing, automatic failover may be blocked.
