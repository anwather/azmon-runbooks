# SQL Server — SQL error 9696 Service Broker event handler cannot start

**Alert ID:** `sql-error-9696-sb-event-handler-cannot-start`
**Severity:** 2 Error
**Category:** Service Broker
**Detection mechanism:** Custom Text Logs (ERRORLOG) DCR
**Source SCOM monitor / rule (if any):** `EventRule.DBEngine.SQL_Server_Service_Broker_event_handler_cannot_start_5_Rule`

---

## Symptom

The proposed alert fires when SQL ERRORLOG ingestion sees error 9696. Typical message text: "The Service Broker event handler could not start."
...
- Microsoft docs: https://learn.microsoft.com/sql/relational-databases/errors-events/mssqlserver-9696-database-engine-error
