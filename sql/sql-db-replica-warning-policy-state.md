# SQL Server — Database replica user policy warning

**Alert ID:** `sql-db-replica-warning-policy-state`
**Severity:** 3 Warning
**Category:** AlwaysOn
**Detection mechanism:** DMV probe
**Source SCOM monitor / rule (if any):** `Monitor.DatabaseReplicaWarningPolicy.State`

---

## Symptom

A custom Policy-Based Management policy for a database replica evaluated to warning.

## What it means

The customer-defined DB replica warning policy has failed and may indicate a configuration drift or degraded replica condition.
