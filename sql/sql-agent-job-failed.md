# SQL Server — SQL Agent job failed to complete successfully

**Alert ID:** `sql-agent-job-failed`
**Severity:** 2 Error
**Category:** Agent
**Detection mechanism:** DCR-Events XPath
**Source SCOM monitor / rule (if any):** `CollectionRule.Agent.A_SQL_job_failed_to_complete_successfully_1_5_Rule`

---

## Symptom

A SQL Agent job completed with a Failed result.

## What it means

A scheduled job failed. The operational impact depends on the job type: backup chain, ETL, index maintenance, replication, log shipping, or custom automation.
...
| where Source has "SQLSERVERAGENT" and EventID == 208
| where EventData has "Failed" or RenderedDescription has "Failed"
