# SQL Server — SQL Server Agent initiated self-termination

**Alert ID:** `sql-agent-self-termination`
**Severity:** 1 Critical
**Category:** Agent
**Detection mechanism:** DCR-Events XPath
**Source SCOM monitor / rule (if any):** `CollectionRule.Agent.SQLServerAgent_initiating_self_termination_1_5_Rule`

---

## Symptom

SQLServerAgent is initiating self-termination.

## What it means

Agent is shutting itself down because an unrecoverable condition was detected. Jobs and alerts stop immediately.
...
| where Source has "SQLSERVERAGENT" and EventID == 51
