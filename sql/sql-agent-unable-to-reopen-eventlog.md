# SQL Server — SQL Agent unable to reopen local event log

**Alert ID:** `sql-agent-unable-to-reopen-eventlog`
**Severity:** 1 Critical
**Category:** Agent
**Detection mechanism:** DCR-Events XPath
**Source SCOM monitor / rule (if any):** `CollectionRule.Agent.Unable_to_re_open_the_local_eventlog_1_5_Rule`

---

## Symptom

SQL Server Agent is unable to re-open the local event log.

## What it means

Agent cannot write/read Application log records that it uses for alerts and diagnostics. Alerting based on event log content can stop.
...
| where Source has "SQLSERVERAGENT" and EventID == 313
